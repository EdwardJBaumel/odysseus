...
def _resolve_auto_stack_fallback(
    endpoint_url,
    model,
    headers,
    *,
    ...
    from src.auto_stack_router import is_auto_stack_model, resolve_auto_stack

    if not is_auto_stack_model(model) or not (endpoint_url or "").strip():
...
def resolve_task_endpoint(fallback_url=None, fallback_model=None, fallback_headers=None, owner=None):
    ...
    return _resolve_auto_stack_fallback(url, model, headers, owner=owner)
