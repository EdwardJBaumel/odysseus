...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        try:
            ...
            # generic 401/503).
            if not is_auto_stack_model(getattr(sess, "model", None)):
                _recover_empty_session_model(sess, session, owner=owner)
            if not getattr(sess, "model", "").strip():
                raise HTTPException(
                    400,
        ...
        if is_auto_stack_active(sess) and not split_stack_available():
            raise HTTPException(503, SPLIT_STACK_MISSING)

        if is_auto_stack_active(sess):
            try:
                check_auto_stack_ready(sess.endpoint_url or "", owner=owner)
            except AutoStackNotReady as exc:
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                try:
                    if _auto_res:
                        _chat_candidates = [
                            (_auto_res.endpoint_url, _auto_res.model, _auto_res.headers),
                        ] + stack_fallback_candidates(
                            _auto_res,
                            endpoint_url=sess.endpoint_url,
                            headers=sess.headers,
                ...
                try:
                    ...
                    async for chunk in stream_agent_loop(
                        ...
                        owner=_user,
                        fallbacks=[] if _auto_stack else _fallback_candidates,
                        workspace=workspace or None,
                        plan_mode=plan_mode,
                        approved_plan=approved_plan or None,
                        auto_stack_active=_auto_stack,
                        auto_stack_anchor_url=sess.endpoint_url,
                    ):
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                data = json.loads(chunk[6:])
